...
public class Run extends Game{
	...
	public Run(){
		...
		id = String.format("R%02d", autoIncrease);
		this.setGameID(id);
		autoIncrease++;
	...
	public ArrayList<Athlete> selectAthletes(){
		...
		System.out.println("Please select runners from the list, you must select 3-8 athletes:");
		System.out.println("\tID\t\tName\t\tAge\t\tState");
		int i = 0;
		for (Athlete anAthlete : athArray)
		{
			if(anAthlete instanceof Sprinter || anAthlete instanceof SuperAthlete)
			{
				i++;
		...
		do{
			System.out.println("Please select one runner at first:");
			select = sc.nextInt();
		...
		do{
			System.out.println("Please select the second runner:");
			select = sc.nextInt();
			if(select <= 0 || select > i)
			{
				System.out.println("Please input a valid number.");
		...
		resultArray.add(tempArray.get(select-1));
		do{
			System.out.println("Please select the third runner:");
			select = sc.nextInt();
			if(select <= 0 || select > i)
			{
				System.out.println("Please input a valid number.");
		...
		do{
			System.out.println("If you want to select one more runner, input his number, otherwise input 0:");
			select = sc.nextInt();
			if(select < 0 || select > i)
			{
				System.out.println("Please input a valid number.");
