...
public class Swim extends Game{
	...
	public Swim(){
		...
		id = String.format("S%02d", autoIncrease);
		this.setGameID(id);
		autoIncrease++;
	...
	public ArrayList<Athlete> selectAthletes(){
		...
		System.out.println("Please select swimmers from the list, you must select 3-8 athletes:");
		System.out.println("\tID\t\tName\t\tAge\t\tState");
		int i = 0;
		for (Athlete anAthlete : athArray)
		{
			if(anAthlete instanceof Swimmer || anAthlete instanceof Sprinter || anAthlete instanceof SuperAthlete)
			{
				i++;
		...
		do{
			System.out.println("Please select one swimmer at first:");
			select = sc.nextInt();
		...
		do{
			System.out.println("Please select the second swimmer:");
			select = sc.nextInt();
			if(select <= 0 || select > i)
			{
				System.out.println("Please input a valid number.");
		...
		resultArray.add(tempArray.get(select-1));
		do{
			System.out.println("Please select the third swimmer:");
			select = sc.nextInt();
			if(select <= 0 || select > i)
			{
				System.out.println("Please input a valid number.");
		...
		do{
			System.out.println("If you want to select one more swimmer, input his number, otherwise input 0:");
			select = sc.nextInt();
			if(select < 0 || select > i)
			{
				System.out.println("Please input a valid number.");
